...
2. **下载到受管源码工作区**
   ...
   ⚠️ **禁止直接 `git clone` 到工作区任意位置。** 所有源码必须落在受管根目录并写入标记文件，否则后续无法被安全自动清理，会持续占用磁盘。

   ```bash
   # 1) 创建受管工作区（命名：{时间戳}-{仓库名}）
   SRC=".cache/sources/$(date +%Y%m%d-%H%M%S)-{repo-name}"
   mkdir -p "$SRC"

   # 2) 浅克隆（--depth 1 省 60%+ 空间，分析不需要提交历史）
   git clone --depth 1 https://github.com/xxx/xxx.git "$SRC"

   # 3) 写入标记文件（清理工具的准入凭证，缺此文件永不自动删除）
   ...
   $src = ".cache/sources/$stamp-{repo-name}"
   ...
   status=in-use
   "@ | Set-Content -LiteralPath "$src/.skill-workspace-source" -Encoding UTF8
   ```

   **只需 SKILL.md 时不要克隆整个仓库：**
...
4. **分析完成后释放**
   - 分析/提取完成 → 执行 `release {工作区名}` 标记为可释放
...
```bash
# 克隆仓库（使用 moeyy 镜像）
# ⚠️ 必须落到受管工作区 + --depth 1 + 写标记文件，详见 source-lifecycle.md
SRC=".cache/sources/$(date +%Y%m%d-%H%M%S)-repo"
mkdir -p "$SRC"
git clone --depth 1 https://github.moeyy.xyz/owner/repo.git "$SRC"
printf 'marker=skill-workspace-source\nsource_url=%s\npurpose=search\ncloned_at=%s\nstatus=in-use\n' \
  "https://github.moeyy.xyz/owner/repo.git" "$(date -Iseconds)" > "$SRC/.skill-workspace-source"
