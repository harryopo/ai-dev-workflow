#!/usr/bin/env python3
"""
Deep Research — 多源搜索引擎
整合 Tavily（AI 搜索）+ Jina Reader（网页提取）+ SearXNG（元搜索）
"""
...
class TavilySearch:
    ...
    def search(self, query: str, search_depth: str = "basic",
        ...
        Args:
            query: 搜索关键词
            search_depth: 搜索深度 (basic/fast/advanced)
            max_results: 最大结果数
            include_answer: 是否包含 AI 生成的回答
...
def multi_search(query: str, max_results: int = 10,
                 sources: list = None, search_depth: str = "basic") -> dict:
    """
    多源并发搜索
    ...
    Args:
        ...
        sources: 搜索源列表 (tavily/jina/searxng)
    ...
    for source in sources:
        ...
        if engine and engine.is_available():
            ...
            print(f"⚠️  {source} 不可用，跳过", file=sys.stderr)
