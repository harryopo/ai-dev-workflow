...
## 4. v3.2 增强章节（基于用户反馈 2026-07-22 · 减法重构）

> **v3.2 关键变化：** v3.1 强制每个项目都沉淀所有知识 + 知识四层架构被证明**对小型项目过于复杂**。v3.2 改为按项目规模选择知识沉淀深度。

### 4.1 阶段 6 的规范应用强度（v3.2 重构）

| 规范 | v3.1 状态 | v3.2 状态 | 何时强化 | 何时弱化 |
|------|----------|----------|---------|---------|
| 知识四层架构 | 🔴 强制 | 🟡 软性建议 | 大型项目、长期项目 | 小项目、一次性脚本 |
| .learnings/ 必填 | 🔴 必填 | 🟡 推荐 | 长期项目 | 原型项目 |
| 每阶段必沉淀 | 🔴 必填 | 🟡 推荐 | 关键决策点 | 常规开发 |
| ADR 必填 | 🔴 必填 | 🟡 关键决策必填 | 长期项目、多人协作 | 单人项目 |
| Context Engineering 4 层 | 🔴 强制 | 🟡 推荐 | 大型项目 | 小项目 |
...
Q: 这个项目需要多深知识沉淀？
A:
  ├─ 大型项目（> 3 个月、> 5 个模块）→ 完整四层架构 + ADR + .learnings/
  ├─ 中型项目（1-3 个月、3-5 个模块）→ 3 层架构 + 关键 ADR
  └─ 小型项目（< 1 个月、单模块）→ 1-2 层架构（CLAUDE.md + 关键决策）
```

### 4.3 阶段 6 反模式（v3.2 新增）

**AP-16 规范过度严苛：** 阶段 6 强制所有项目都沉淀四层架构 → 300 行单文件 CLI 也要写 4 层

**AP-23 知识沉淀仪式化：** 为了沉淀而沉淀，内容空洞无价值
...
A: 取决于 3 个问题：
  ...
  2. 团队规模？（单人可选关键沉淀，团队需完整）
  3. 复用需求？（跨项目复用需完整，单项目可简化）
```
